...
static void MuteKartSounds(Audio::EngineMgr* mgr, Audio::KartActor* actor) {
    mgr->Init(actor);
    if (System::sInstance->IsContext(PULSAR_MODE_OTT) && !actor->isLocal && !Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_OTT, RADIO_OTTMUTEPTANDPLAYERS)) {
        actor->isGhost = true;
    }
}
...
static bool MuteCharSounds(Kart::Link* link) {
    ...
    if (System::sInstance->IsContext(PULSAR_MODE_OTT) && !Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_OTT, RADIO_OTTMUTEPTANDPLAYERS)) {
...
static bool MutePositionTracker(CtrlRaceRankNum& tracker) {  // isInactive = muted if true, bctrl = IsInactive
    ...
    if (System::sInstance->IsContext(PULSAR_MODE_OTT) && isInactive) isInactive = Settings::Mgr::Get().GetSettingValue(Settings::SETTINGSTYPE_OTT, RADIO_OTTMUTEPTANDPLAYERS);
    return isInactive;
}
...
static void ClearOTTOnOfflineMenus() {
    System* system = System::sInstance;
...
static SectionLoadHook clearOTT(ClearOTTOnOfflineMenus);
